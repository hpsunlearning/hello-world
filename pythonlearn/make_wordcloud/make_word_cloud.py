...
d = path.dirname('.')
stopwords = dict()
importStopword(filename=path.join(d, 'stopwords.txt'))

text = open(path.join(d, 'wukong.txt'),encoding ='utf-8').read()
#text = processChinese(text)#对中文分词

back_coloring = imread(path.join(d, "wukong.jpg")) #一般配色

wc = WordCloud(
#                font_path =path.join(d, 'font.ttf'),#设置字体
                background_color="white",#设置背景颜色
                mask=back_coloring,#设置背景图片
#                stopwords=STOPWORDS.add("said"),#设置停用词，默认STOPWORDS
                max_words=2000,
                max_font_size=40,
                width=800,#设置图片大小
                height=600,
