library(Cairo)
argv <- commandArgs(T)
d <- read.table(argv[1],row.names=1)
outdir <- argv[2]
dat <- d["Firmicutes",]/d["Bacteroidetes",]
...
rownames(stat) <- "Firmicutes/Bacteroidetes"
write.table(stat,paste(outdir,"/","ratio.Firmicutes_Bacteroidetes.stat",sep=""),quote=F,col.names=NA)

CairoPDF(paste(outdir,"/","ratio.Firmicutes_Bacteroidetes",".pdf",sep=""),width=1.875*3,height=0.278*3)
