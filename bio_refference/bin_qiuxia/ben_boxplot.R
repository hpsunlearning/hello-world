...
sam_name <- colnames(d)
quan <- t(apply(d,1,function(x){quantile(x,seq(0,1,0.001))}))
bac_name <- rownames(quan)
for (j in 1:nrow(d)){for (i in 1:ncol(d)){ 
	ab <- abs(quan[bac_name[j],]-d[bac_name[j],sam_name[i]]) 
	min <- min(abs(quan[bac_name[j],]-d[bac_name[j],sam_name[i]]))
	data[j,i] <- as.numeric(gsub("%","",names(ab[max(which(ab==min))])))/100
}}
...
#	par(mar=c(4.2,12.2,2.1,2.1),bg=rgb(red=238,green=236,blue=225,max = 255))
	...
	for (i in 1:nrow(data)){
		...
		for (j in seq(0.2,1,length=1000)){segments(j,i+0.2,j,i-0.2,col=hsv(alpha=0.9,v=0.76,h=1/3),lwd=1.5)}
	}
	abline(v=0.1,lty=4)
	
	for (i in 1:nrow(data)){
		polygon(c(data[i,1]-1/60,data[i,1]-1/60,data[i,1],data[i,1]+1/60,data[i,1]+1/60),c(i+0.3,i+0.2,i,i+0.2,i+0.3),col=hsv(h=0,s=0,v=0.45),border=0)	}
	
	axis(side=1,at=0.05,lwd=0,lwd.ticks=0,tck=-0.03,labels="含量偏低",font=2,las=1)
	axis(side=1,at=0.55,lwd=0,lwd.ticks=0,tck=-0.03,labels="正常",font=2,las=1)
...
#		par(mar=c(4.2,12.2,2.1,2.1),bg=rgb(red=238,green=236,blue=225,max = 255))
		...
		for (j in 1:nrow(data)){
			...
			for (k in seq(0.2,1,length=1000)){segments(k,j-0.2,k,j+0.2,col=hsv(alpha=0.9,v=0.76,h=1/3),lwd=1.5)}
		}
		abline(v=0.1,lty=4)
