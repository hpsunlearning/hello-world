...
ref <- d[,-num]
control_list<-readLines(argv[4])
control_list<-gsub("-",".",control_list)
index <- pmatch(control_list,colnames(ref))
...
for (i in 1:num){
        outName<-paste(outdir,"/",colnames(d)[i],".T2D.speciesmarker.pdf",sep="")
	...
	abline(h=1.5,col=hsv(alpha=0.8,s=0,v=0))
	abline(h=5.5,col=hsv(alpha=0.8,s=0,v=0))
...
write.table(out,paste(outdir,"/","t2d_speciesmarker.stat",sep=""),quote=F,col.names = NA,sep="\t")
