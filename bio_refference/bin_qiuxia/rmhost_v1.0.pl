...
our ($opt_a, $opt_b, $opt_c, $opt_d, $opt_m, $opt_s, $opt_r, $opt_n);
our ($opt_x, $opt_v, $opt_i, $opt_t, $opt_f, $opt_q, $opt_p, $opt_h);
getopts('a:b:c:d:m:s:r:n:x:v:i:t:f:qp:h');
...
$soap_path = "/ifs5/PC_MICRO_META/PRJ/MetaSystem/analysis_flow/bin/program/soap2.22";
@host = split /:/, $opt_d;
$host = join " -D ", @host;
...
if($opt_b){
	$shell = "$soap_path -a $opt_a -b $opt_b -D $host -M $opt_m -l $opt_s -r $opt_r -m $opt_n -x $opt_x -v $opt_v -c $opt_i -p $opt_t ";
	if($opt_f eq "Y"){
		$shell .= "-S ";
	}
	...
	if($opt_c){
		$shell = "$soap_path -a $opt_c -D $host -M $opt_m -l $opt_s -r $opt_r -v $opt_v -c $opt_i -p $opt_t ";
		if($opt_f eq "Y"){
			$shell .= "-S ";
		}
	...
	$shell = "$soap_path -a $opt_a -D $host -M $opt_m -l $opt_s -r $opt_r -v $opt_v -c $opt_i -p $opt_t ";
	if($opt_f eq "Y"){
		$shell .= "-S ";
	}
...
sub get_remove{
	my $file = shift;
	open I, "<:gzip(autopop)", "$file" or die "can't open file $file $!\n";
	while(<I>){
		chomp;
		my @temp = split /\s+/, $_;
		$temp[0] =~ s/\/[12]$//;
...
sub get_paired_fq{
	...
	open I1, "<:gzip(autopop)", "$in1" or die "can't open file $in1 $!\n";
	open I2, "<:gzip(autopop)", "$in2" or die "can't open file $in2 $!\n";
...
sub get_single_fq{
	my ($in, $out) = @_;
	open I, "<:gzip(autopop)", "$in" or die "can't open file $in $!\n";
