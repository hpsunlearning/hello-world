...
#	par(mar=c(4.2,12.2,2.1,2.1),bg=rgb(red=238,green=236,blue=225,max = 255))
	...
	for (i in 1:nrow(data)){
		for (j in seq(0,0.8,length=1000)){segments(j,i+0.2,j,i-0.2,col=hsv(alpha=0.9,v=0.76,h=1/3),lwd=1.5)}
	...
	axis(side=1,at=0.45,lwd=0,lwd.ticks=0,tck=-0.03,labels="正常",font=2,las=1)
...
#		par(mar=c(4.2,12.2,2.1,2.1),bg=rgb(red=238,green=236,blue=225,max = 255))
		...
		for (j in 1:nrow(data)){
			for (k in seq(0,0.8,length=1000)){segments(k,j-0.2,k,j+0.2,col=hsv(alpha=0.9,v=0.76,h=1/3),lwd=1.5)}
