...
if($opt_b){
	...
	$shell .= "-o $opt_p.abundance.soap 2> $opt_p.abundance.soap.log";
	if(system($shell)){
		print STDERR "single read align database error\n";
		exit(1);
	}
...
if($opt_b){
	...
	&get_abundance_single("$opt_p.abundance.soap");
...
print O "\t$opt_p\n"; # 2014/11/25 add
for($i = 0; $i < @id; $i++){
	print O "$id[$i]\t$readsnum{$name[$i]}\t$abundance{$name[$i]}\t", $abundance{$name[$i]} / $total_abundance, "\n";
}
close O;
